    ...
    accounts.forEach(acc => {
        ...
        if (acc.goals && acc.goals.length > 0) {
            ...
            goalsHtml = `<button class="btn-add-goal" onclick="addGoal(${acc.id})">
                <i class="fas fa-plus"></i> Добавить цель (не влияет на баланс)
            </button>`;
        }

        li.innerHTML = `
            <div class="acc-header">
                <div class="acc-info"><span>${acc.icon}</span><span>${acc.name}</span></div>
...
parseSmsBtn.addEventListener('click', async () => {
    ...
    const keywords = {
        '🛒 Продукты': ['пятерочка', 'магнит', 'ашан', 'вкусвилл'],
        '🚕 Транспорт': ['яндекс', 'такси', 'метро', 'бензин'],
